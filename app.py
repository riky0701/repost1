...
def colors(value):
    ...
    return value
#>>>>>
def log(value, status='', color='[P1]'):
    value = colors('{color}[CC]{status}[Y1] <#> {color}{status}[P1]{color}{value}[CC]'.format(
...
#>>>>>>>>
def log_replace(value, status='', color='[G1]'):
...
class inject(object):
    ...
    def start(self):
        try:
            ...
            frontend_domains = open(real_path('/d.txt')).readlines()
            frontend_domains = filter_array(frontend_domains)
            if len(frontend_domains) == 0:
                self.log('Frontend Domains not found. Please check d.txt', color='[R1]')
                return
...
#>>>>
    ...
    def run(self):
        try:
            ...
            self.socket_client.sendall(b'HTTP/1.1 200 Connected\r\nConnection:Keep-Alive\r\n\r\n')
...
def main():
    print(colors('\n'.join([
        '[P1]###########################################\n[Y1]Domain Fronting [Rabbit Version 1.3.190427]',
        '[Y1](c) 2019 Aztec Rabbit.\n[P1]###########################################', '[CC]'
    ])))
